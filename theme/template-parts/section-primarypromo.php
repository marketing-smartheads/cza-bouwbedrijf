...
// check of promo bruikbare inhoud heeft
$has_promo_content = !empty($promo['title']) || !empty($promo['text']) || !empty($promo['image']) || !empty($promo['link']['url']);
?>

<?php if ($has_promo_content): ?>
        ...
        <div class="promo__container">
            <div class="promo__content" data-animate="fade-in-up">
                <?php if (!empty($promo['title'])): ?>
                    <h2 id="promo-title" class="promo__title"><?= esc_html($promo['title']) ?></h2>
                <?php endif; ?>

                <?php if (!empty($promo['text'])): ?>
                    <div class="promo__text"><?= wp_kses_post($promo['text']) ?></div>
                <?php endif; ?>

                <?php if (!empty($promo['link']['url'])): ?>
                    <a href="<?= esc_url($promo['link']['url']) ?>" class="promo__cta button button--primary">
            ...
            <?php if (!empty($promo['image'])): ?>
                <figure class="promo__image-wrapper">
